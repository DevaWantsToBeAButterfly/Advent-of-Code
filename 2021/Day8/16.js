...
function resolveSixNine(inputLine, segmentMap) {
	zeroSixNineSequences = inputLine.filter(litSegments => litSegments.length === 6)
	sixNineSequences = []

	segmentFiveSevenValues = segmentMap.get(7)

	for (sequence of zeroSixNineSequences) {
		if (sequence.includes(segmentMap.get(4))) {
			sixNineSequences.push(sequence)
		}
	}

	for (letter of segmentFiveSevenValues) {
		if (sixNineSequences[0].includes(letter) && sixNineSequences[1].includes(letter)) {
			segmentMap.set(7, letter)
		} else {
			segmentMap.set(5, letter)
		}
	}

	segmentThreeSixValues = segmentMap.get(3)

	for (letter of segmentThreeSixValues) {
		if (sixNineSequences[0].includes(letter) && sixNineSequences[1].includes(letter)) {
			segmentMap.set(6, letter)
		} else {
			segmentMap.set(3, letter)
		}
	}
...
function translateToInt(outputLine, segmentMap) {
	outputNumber = ''
	for (output of outputLine) {
		litSegments = []
		for (letter of output) {
			litSegment = segmentMap.get(letter)
			litSegments.push(litSegment)
		}
		litSegments = litSegments.sort()
		litSegments = litSegments.join('')
		intValue = litSegmentsToIntMap.get(litSegments)
		outputNumber += intValue
...
function resetMap() {
	...
	for (let i = 1; i <= 7; i++) {
		segmentMap.set(i, 'abcdefg')
	}

	return segmentMap
}

translateOutput(puzzleInput)
