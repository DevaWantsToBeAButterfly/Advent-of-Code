...
function playTurn(drawnNumber) {
	...
	for (wonBoard of wonBoards) {
		boards.delete(wonBoard)
	}
	wonBoards.clear()
}
...
function playGame(draws) {
	for (draw of draws) {
		if (boards.size) {
			console.log('Randomly drew number ' + draw)
			playTurn(draw)
		} else {
			console.log('We have found our last winner')
			endGame(latestWinner)
			return
		}
...
function endGame(winner) {
	console.log('Game Over! The last board to win is number ' + winner.id)
	countScore(winner)
}

function countScore(winner) {
	for (number of winner.numbers) {
		if (pastDraws.includes(number)) {
			console.log(number + ' was drawn, so it is not counted')
		} else {
			console.log('The current total is ' + unmarkedTotal + ', and we just added ' + number + ' to it for a temporary total of ' + (unmarkedTotal + number))
			unmarkedTotal += number
		}
	}
	finalScore = unmarkedTotal * pastDraws.slice(pastDraws.length - 1)
	console.log('The final score is ' + finalScore)
	// console.log(pastDraws)
}

gameSetup(boardsInput)
playGame(drawsInput)