...
function drawMap(inputArray) {
	let highestXCoord = 0
	let highestYCoord = 0

	for (pair of inputArray) {
		for (coordSet of pair) {
			if (coordSet[0] > highestXCoord) {
				highestXCoord = coordSet[0]
				// console.log('The new highest X coordinate is ' + highestXCoord)
			}
			if (coordSet[1] > highestYCoord) {
				highestYCoord = coordSet[1]
				// console.log('The new highest Y coordinate is ' + highestYCoord)
			}
		}
	}

	for (let tempXCoord = 0; tempXCoord <= highestXCoord; tempXCoord++) {
		for (let tempYCoord = 0; tempYCoord <= highestYCoord; tempYCoord++) {
			let newMapLocation = new MapLocation(tempXCoord, tempYCoord)
			mapLocations.push(newMapLocation)
		}
	}
...
function findPoison(inputArray, mapArray) {
	for (pair of inputArray) {
		console.log('Checking input ' + inputArray.indexOf(pair) + ' of ' + inputArray.length)
		checkPoison = true
		startLocation = pair[0]
		endLocation = pair[1]
		...
		if (startLocation[0] === endLocation[0]) {
			fixedCoord = startLocation[0]
			movementStart = Math.min(startLocation[1], endLocation[1])
			movementEnd = Math.max(startLocation[1], endLocation[1])

			for (let movementIndex = movementStart; movementIndex <= movementEnd; movementIndex++) {
				let locationId = (fixedCoord * 10000 + movementIndex)
				for (location of mapArray) {
					if (location.id === locationId) {
						location.poisonCounter++
						break
					}
				}
			}
			...
			fixedCoord = startLocation[1]
			movementStart = Math.min(startLocation[0], endLocation[0])
			movementEnd = Math.max(startLocation[0], endLocation[0])

			// console.log('There is a straight line of poison going from ' + movementStart + ',' + fixedCoord + ' to ' + movementEnd + ',' + fixedCoord)

			for (let movementIndex = movementStart; movementIndex <= movementEnd; movementIndex++) {
				let locationId = (movementIndex * 10000 + fixedCoord)

				// console.log('Poison found on cell ' + locationId + ', at coordinates ' + movementIndex + ',' + fixedCoord)
				for (location of mapArray) {
					// console.log('Checking if location ' + location.id + ' is the one we are looking for. MovementEnd is ' + movementEnd)
					if (location.id === locationId) {
						location.poisonCounter++
						break
					}
...
function reportBadlyPoisoned(mapArray) {
	for (location of mapArray) {
		if (location.poisonCounter > 1) {
			dangerousLocations.push(location)
		}
	}
	console.log(dangerousLocations)
	console.log('There are a total of ' + dangerousLocations.length + ' dangerous locations')
}
drawMap(realInput)
findPoison(realInput, mapLocations)
reportBadlyPoisoned(mapLocations)
