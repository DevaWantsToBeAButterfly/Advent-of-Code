...
let dangerousLocations = new Map()
let currentLine = 0

function findPoison(inputArray, checkDiagonals) {
	for (pair of inputArray) {
		startLocation = pair[0]
		endLocation = pair[1]

		firstX = startLocation[0]
		firstY = startLocation[1]
		secondX = endLocation[0]
		secondY = endLocation[1]

		console.log('Checking input ' + startLocation + ' => ' + endLocation)

		if (firstX === secondX) {
			fixedCoord = firstX
			movementStart = Math.min(firstY, secondY)
			movementEnd = Math.max(firstY, secondY)

			console.log('Detected horizontal line going from ' + startLocation + ' to ' + endLocation)

			for (movementStart; movementStart <= movementEnd; movementStart++) {
				let locationId = (fixedCoord * 10000 + movementStart)
				locationPoison = worldMap.get(locationId)
				if (locationPoison) {
					worldMap.set(locationId, (locationPoison + 1))
				} else {
					worldMap.set(locationId, 1)
				}
			}

		} else if (firstY === secondY) {
			fixedCoord = firstY
			movementStart = Math.min(firstX, secondX)
			movementEnd = Math.max(firstX, secondX)

			console.log('Detected vertical line going from ' + startLocation + ' to ' + endLocation)

			for (movementStart; movementStart <= movementEnd; movementStart++) {
				let locationId = (movementStart * 10000 + fixedCoord)
				locationPoison = worldMap.get(locationId)
		...
		} else if (checkDiagonals && Math.abs(firstX - secondX) === Math.abs(firstY - secondY)) {

			console.log('Detected diagonal line going from ' + startLocation + ' to ' + endLocation)

			let verticalMovement = firstX > secondX ? -1 : 1

			let horizontalMovement = firstY > secondY ? -1 : 1
			...
			while (true) {
				...
				// console.log('Recording poison at ' + currentX + ',' + currentY)
				locationId = currentX * 10000 + currentY
				locationPoison = worldMap.get(locationId)
				...
				currentX += verticalMovement
				currentY += horizontalMovement
			}
		}
	}

	for (const [key, value] of worldMap.entries()) {
		if (value >= 2) {
			dangerousLocations.set(key, value)
		}
	}
	console.log(dangerousLocations.size)
}
...
// 		let mapLine = ''
// 		for (let y = 0; y <= 9; y++) {
// 			currentLocId = x * 10000 + y
// 			currentValue = worldMap.get(currentLocId)
// 			if (currentValue) {
// 				asString = currentValue.toString()
// 				mapLine += x.toString() + ',' + y.toString() + ' ' + asString + ' //'
// 			} else {
// 				mapLine += x.toString() + ',' + y.toString() + ' 0 //'
// 			}
